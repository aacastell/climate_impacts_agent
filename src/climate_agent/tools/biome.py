...
SECTOR_IMPACT_TEXT = "Elevated probability of biome transition toward drier vegetation (stub — real tool not wired yet)."


def compute_biome(bbox: BBox, gwl: str) -> tuple[list[GridCell], str]:
    """Change-vs-baseline biome-shift impact grid and summary text.
    ...
    impact_grid = [
        GridCell(lat=lat, lon=lon, value=stub_value("impact:biome", lat, lon, 0.0, 1.0))
